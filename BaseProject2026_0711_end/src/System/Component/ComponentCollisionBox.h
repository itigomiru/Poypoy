...
public:
	...
	{
		collision_type_ = CollisionType::BOX;
	}
	ComponentCollisionBoxPtr SetName(const std::string& name)
	{
		return Component::SetName<ComponentCollisionBox>(name);
	}
	...
	virtual ComponentCollisionBoxPtr SharedThis() override
	{
	...
	static int GetSharedBoxModel()
	{
		return shared_box_model_;
	}
